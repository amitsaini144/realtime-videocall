...
export default function MessageSection({ receivedMessages, messagesEndRef }: Readonly<MessageSectionProps>) {
  return (
      ...
      <div className="flex-grow overflow-y-auto bg-white rounded-2xl p-4 scrollbar-thin">
        {receivedMessages.length === 0 ? (
          ...
          receivedMessages.map((message) => (
            <div key={message.id} className={`mb-3 flex items-end gap-2 ${message.isOwn ? 'justify-end' : 'justify-start'}`}>
              {!message.isOwn && (
